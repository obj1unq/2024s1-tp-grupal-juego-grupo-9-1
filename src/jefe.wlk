...
object jefe {
	...
	method mover(){
		...
		if (direcciones.esUnBorde(self.position())) {
			moviendoA = moviendoA.opuesto()
			self.position(moviendoA.siguiente(self.position()))
		}else{
			self.position(moviendoA.siguiente(self.position()))
		}
	}
    ...
    method escudos(){
    	return if (self.cantidadEscudos() > 0) self.cantidadEscudos().toString()
    	else "Vulnerable"
    }
	...
	method recibirDanio(cantidad){
		...
		else {aguante = (aguante - cantidad).max(0)
